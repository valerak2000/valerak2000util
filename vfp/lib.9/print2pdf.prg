parameters pcFileName, pcReport, pcExtraRptClauses, pcPSFile, pcGSFolder

**********************************************************************************************************
* Print2PDF.PRG  Version 1.50
*
...
IF !EMPTY(m.pcExtraRptClauses)
	loPDF.cExtraRptClauses	= m.pcExtraRptClauses
ENDIF

IF !EMPTY(m.pcGSFolder)
	loPDF.cGSFolder=m.pcGSFolder
ENDIF

loPDF.cPSFile		= pcPSFile
llResult 			= loPDF.Main()
...
*!*				this.cPSPrinter	= "CUTEPDF WRITER"
		endif
		if empty(this.cPSColorPrinter)
	...
	function GSConvertFile(tcFileIn, tcFileOut)
		...
		*lcProg = alltrim(this.cGSFolder) + "..\lib\ps2pdf.bat"
		...
		lcParms = 	"-sDEVICE=pdfwrite"+;
					...
					" -dNOPAUSE"+;
					" -q"+;
					" -c 3000000 setvmthreshold .setpdfwrite -f"+tcFileIn
					
		lcCmd=lcProg+' '+lcParms

susp
		LOCAL loShell as Object, loExc as Exception
		*создание архива
		TRY
			SET DEFAULT TO THIS.cGSFolder

			loShell=CREATEOBJECT("WScript.Shell")
			loShell.Run(m.lcCmd , 0, .T.)
		CATCH TO loExc
			m.goApp.Function.ShowErrMsg(m.loExc)
		FINALLY
			SET DEFAULT TO m.goApp.Vars.cAppCurPaths
		ENDTRY
