...
*!*		ENDIF
	...
	CREATE CURSOR PRINT_REP (DATE D, KOLN N(13, 6), KOLNJIR N(13, 6),;
							NAME_PRIH C(70), KOL_PRIH N(13, 6), KOLJIR_PRIH N(13, 6),;
							NAME_RASH C(70), KOL_RASH N(13, 6), KOLJIR_RASH N(13, 6),;
							KOLK N(13, 6), KOLKJIR N(13, 6), SOOTN N(13, 6), EDNAME C(30), FLAG N(1))
	INDEX ON DTOS(DATE)+TRANSFORM(FLAG, '9')+IIF(!EMPTY(NAME_PRIH), '0', '9')+IIF(!EMPTY(NAME_RASH), '0', '9')+EDNAME TAG dt

	CREATE CURSOR PRINT_REPPR (DATE D, NAME_PRIH C(70), KOL_PRIH N(13, 6), KOLJIR_PRIH N(13, 6),;
	...
	SELECT DISTINCT 0, NVL(PRODUCTION.ID, 0000000000), PADR(RTRIM(NVL(PRODUCTION.COMMENT, ''))+CHR(13)+RTRIM(NVL(TYPEDOC.NAME, ''))+CHR(13)+RTRIM(NVL(OR.NAME, '')), 70), NVL(EDIZM.ID, 0000000000), NVL(OR.ID, 0000000000), NVL(TYPEDOC.ID, 0000000000), NVL(EDIZM.NAME, SPACE(40)), 1;
	...
	UNION ALL;
	SELECT DISTINCT 1, NVL(TYPEDOC.ID, 0000000000), PADR(RTRIM(NVL(TYPEDOC.NAME, ''))+CHR(13)+RTRIM(NVL(OR.NAME, '')), 70), NVL(EDIZM.ID,0000000000), NVL(OR.ID, 0000000000), NVL(TYPEDOC.ID, 0000000000), NVL(EDIZM.NAME, SPACE(40)), 1;
		...
		FOR lnJ=1 TO ALEN(laproview, 1)
			...
			IF VARTYPE(laproview[m.lnJ, 1])='N'
				...
				IF lasumviewprih[1, 1]<>0
					...
					INSERT INTO PRINT_REPPR (DATE, NAME_PRIH, KOL_PRIH, KOLJIR_PRIH, SOOTN, EDNAME);
				...
				IF lasumviewrash[1, 1]<>0
					...
					INSERT INTO PRINT_REP (DATE, NAME_RASH, KOL_RASH, KOLJIR_RASH, SOOTN, EDNAME);
	...
	DO WHILE ldI<=m.tddateend
		...
		*добавляем записи из приходов
			INSERT INTO PRINT_REP (DATE, NAME_PRIH, KOL_PRIH, KOLJIR_PRIH, SOOTN, EDNAME);
						SELECT DATE, NAME_PRIH, KOL_PRIH, KOLJIR_PRIH, SOOTN, EDNAME;
		...
		*сделать замену
			...
			SCAN FOR DATE=m.ldI
				IF PRINT_REPPR.DATE<>m.ldI OR PRINT_REPPR.EDNAME<>PRINT_REP.EDNAME OR EOF("PRINT_REPPR")
					LOOP
				ELSE
					REPLACE NAME_PRIH WITH PRINT_REPPR.NAME_PRIH, KOL_PRIH WITH PRINT_REPPR.KOL_PRIH, KOLJIR_PRIH WITH PRINT_REPPR.KOLJIR_PRIH IN PRINT_REP
		...
		*сделать замену+добавляем записи
			...
			SCAN FOR DATE=m.ldI
				IF PRINT_REP.DATE<>m.ldI OR PRINT_REP.EDNAME<>PRINT_REPPR.EDNAME OR EOF("PRINT_REP")						
					INSERT INTO PRINT_REP (DATE, NAME_PRIH, KOL_PRIH, KOLJIR_PRIH, SOOTN, EDNAME);
								   VALUES (PRINT_REPPR.DATE, PRINT_REPPR.NAME_PRIH, PRINT_REPPR.KOL_PRIH, PRINT_REPPR.KOLJIR_PRIH, PRINT_REPPR.SOOTN, PRINT_REPPR.EDNAME)
				ELSE
					REPLACE NAME_PRIH WITH PRINT_REPPR.NAME_PRIH, KOL_PRIH WITH PRINT_REPPR.KOL_PRIH, KOLJIR_PRIH WITH PRINT_REPPR.KOLJIR_PRIH IN PRINT_REP
				ENDIF
	...
	*найти дату до m.tddatebeg на которую есть остатки m.tddatebeg-1
	INSERT INTO PRINT_REP (DATE, NAME_PRIH, KOLK, KOLKJIR, SOOTN, EDNAME);
				SELECT m.tddatebeg-1, "Входящий остаток", SUM(a.KOLK) as koln, SUM(a.KOLKJIR) as koljir, 1, EDIZM.NAME;
	...
	SCAN &&FOR BETWEEN(DATE, m.tddatebeg, m.tddateend)
		FOR lnI=1 TO ALEN(laostkoln, 1)
			IF EDNAME==laostkoln[m.lnI, 1]
				IF DATE>=m.tddatebeg
					REPLACE KOLN WITH laostkoln[m.lnI, 2], KOLNJIR WITH laostkoln[m.lnI, 3],;
						    ...
						    KOLKJIR WITH laostkoln[m.lnI, 3]+KOLJIR_PRIH-IIF(INLIST(m.tnidtovar, 1, 2) AND ATC("Расход потери при реализации", NAME_RASH)<>0, 0, KOLJIR_RASH)
				ENDIF

				laostkoln[m.lnI, 2]=PRINT_REP.KOLK
				laostkoln[m.lnI, 3]=PRINT_REP.KOLKJIR
			ENDIF
		ENDFOR
	ENDSCAN
	*заполнить ИТОГИ ПО ДЕКАДАМ
	...
	DO WHILE ldI<=m.tddateend
	    ...
	    IF m.lnJ>9 OR m.ldI=m.tddateend
			...
			SELECT NAME_PRIH, SUM(KOL_PRIH), SUM(KOLJIR_PRIH), MIN(SOOTN) as sootn, EDNAME, '', 0, 0, 0, '';
			FROM PRINT_REP INTO ARRAY larcursgroup;
			WHERE BETWEEN(DATE, m.ldOldD, m.ldI) AND !EMPTY(NAME_PRIH);
			GROUP BY NAME_PRIH, EDNAME;
			ORDER BY 1, 6

			SELECT NAME_RASH, MIN(SOOTN) as sootn, EDNAME, SUM(KOL_RASH) as kolrh, SUM(KOLJIR_RASH) as koljirrh;
			...
			SELECT EDNAME, SUM(KOLK), SUM(KOLKJIR);
			FROM PRINT_REP INTO ARRAY laostkoln;
			...
			SELECT EDNAME, DATE, KOLK, KOLKJIR;
			...
			FOR lnK=1 TO ALEN(larcursgroup, 1)
				IF !(EMPTY(larcursgroup[m.lnK, 1]) AND EMPTY(larcursgroup[m.lnK, 7]))
					STORE 0 TO lnkoln, lnkolnjir

					FOR lnI=1 TO ALEN(laostkoln, 1)
						IF larcursgroup[m.lnK, 10]==laostkoln[m.lnI, 1]
							lnkoln=laostkoln[m.lnI, 2]
							lnkolnjir=laostkoln[m.lnI, 3]
						ENDIF
					ENDFOR

					INSERT INTO PRINT_REP (DATE, NAME_PRIH, KOL_PRIH, KOLJIR_PRIH,;
										   NAME_RASH, KOL_RASH, KOLJIR_RASH, KOLK, KOLKJIR, SOOTN, EDNAME, FLAG);
								   VALUES (m.ldI, larcursgroup[m.lnK, 1], larcursgroup[m.lnK, 2], larcursgroup[m.lnK, 3],;
								   		   larcursgroup[m.lnK, 6], larcursgroup[m.lnK, 7], larcursgroup[m.lnK, 8], m.lnkoln, m.lnkolnjir, larcursgroup[m.lnK, 9], larcursgroup[m.lnK, 10], 1)
				ENDIF
			ENDFOR
	...
	FOR lnK=1 TO ALEN(larcursgroup, 1)
		...
		INSERT INTO PRINT_REP (DATE, NAME_PRIH, KOL_PRIH, KOLJIR_PRIH,;
					   ...
					   VALUES (m.tddateend, larcursgroup[m.lnK, 1], larcursgroup[m.lnK, 2], larcursgroup[m.lnK, 3],;
					   		   larcursgroup[m.lnK, 6], larcursgroup[m.lnK, 7], larcursgroup[m.lnK, 8], m.lnkoln, m.lnkolnjir, larcursgroup[m.lnK, 9], larcursgroup[m.lnK, 10], 2)
	ENDFOR
    *Total
    DIMENSION larcursgroup[1, 1]
	larcursgroup[1, 1]=''

	SELECT 'ОБЩИЙ ИТОГ' as NAME_PRIH, SUM(KOL_PRIH), SUM(KOLJIR_PRIH), MIN(SOOTN) as sootn, EDNAME, '', 0, 0, 0, '';
	FROM PRINT_REP INTO ARRAY larcursgroup;
	WHERE BETWEEN(DATE, m.tddatebeg, m.tddateend) AND !EMPTY(NAME_PRIH) AND FLAG=0;
	GROUP BY EDNAME;
	ORDER BY 1, 6

	SELECT 'ОБЩИЙ ИТОГ' as NAME_RASH, MIN(SOOTN) as sootn, EDNAME, SUM(KOL_RASH) AS KOLRH, SUM(KOLJIR_RASH) as koljirrh;
	FROM PRINT_REP INTO CURSOR CURSGROUP;
	...
	FOR lnK=1 TO ALEN(larcursgroup, 1)
		...
		INSERT INTO PRINT_REP (DATE, NAME_PRIH, KOL_PRIH, KOLJIR_PRIH,;
							   NAME_RASH, KOL_RASH, KOLJIR_RASH, KOLK, KOLKJIR, SOOTN, EDNAME, FLAG);
					   VALUES (m.tddateend, larcursgroup[m.lnK, 1], larcursgroup[m.lnK, 2], larcursgroup[m.lnK, 3],;
					   		   larcursgroup[m.lnK, 6], larcursgroup[m.lnK, 7], larcursgroup[m.lnK, 8], m.lnkoln, m.lnkolnjir, larcursgroup[m.lnK, 9], larcursgroup[m.lnK, 10], 3)
	ENDFOR
